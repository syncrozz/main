  ...
  return (
    <section id="platform" className="py-16 md:py-24 bg-white relative">
      <div className="max-w-7xl mx-auto px-4 sm:px-8 lg:px-10">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-8 gap-4">
          <div className="text-left space-y-2">
            <div className="flex items-center gap-2">
              ...
              {isAdminMode && (
                <span className="text-[11px] font-bold tracking-wider text-amber-700 bg-amber-100 px-3 py-1 rounded-full inline-flex items-center gap-1 border border-amber-200 animate-pulse">
            ...
            <p className="text-sm sm:text-base text-slate-500 font-normal">
              Terokai ekosistem penyelesaian digital SYNCROZZ. Lengkap dengan sokongan Open Graph Image (JPG) bagi setiap kad.
            </p>
          </div>
          ...
          <div className="flex items-center gap-2.5 shrink-0">
            ...
            <button
              ...
              className="p-2.5 rounded-xl border border-blue-200 bg-blue-50/80 hover:bg-blue-100 text-[#0056D2] transition-all flex items-center gap-2 text-xs font-bold cursor-pointer shadow-2xs"
            >
            ...
            <button 
              ...
              className="text-sm font-bold text-[#0056D2] flex items-center gap-1 hover:text-blue-700 transition-colors cursor-pointer"
            >
        ...
        {isAdminMode && (
          <div className="mb-6 p-4 rounded-2xl bg-gradient-to-r from-amber-50 via-orange-50 to-blue-50 border border-amber-200/80 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 text-left">
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="text-xs font-bold text-amber-950 flex items-center gap-1.5">
                  <span>Mod Pentadbir: Ruang Muat Naik Open Graph Image (JPG) Dibuka</span>
                  <span className="px-1.5 py-0.5 rounded text-[10px] bg-amber-200/80 text-amber-900 font-mono">1200×630</span>
                </div>
                <div className="text-[11px] text-amber-800">
                  Anda boleh klik pada butang <strong>"Muat Naik JPG"</strong> di setiap kad untuk menggantikan gambar Open Graph platform.
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2 w-full sm:w-auto">
              <button
                ...
                className="flex-1 sm:flex-none px-4 py-2 rounded-xl bg-[#0056D2] text-white text-xs font-bold hover:bg-blue-700 transition-colors flex items-center justify-center gap-1.5 cursor-pointer shadow-xs"
              >
              ...
              <button
                ...
                className="px-3 py-2 rounded-xl bg-white border border-amber-300 text-amber-900 text-xs font-semibold hover:bg-amber-100 transition-colors cursor-pointer"
              >
                Tutup Mod
              </button>
            </div>
        ...
        {filteredPlatforms.length === 0 ? (
          <div className="text-center py-16 bg-slate-50 rounded-2xl border border-dashed border-slate-200">
            ...
            <button
              onClick={() => { setSelectedCategory('All'); setSearchQuery(''); }}
              className="mt-3 text-xs font-semibold text-[#0056D2] hover:underline cursor-pointer"
            >
              Reset tapisan
            </button>
          </div>
            ...
            {filteredPlatforms.map((item) => {
              ...
              return (
                <div
                  ...
                  className="group relative bg-white rounded-2xl border border-slate-100 hover:border-blue-200 hover:shadow-lg transition-all duration-200 flex flex-col justify-between overflow-hidden text-left"
                >
                  {/* Card Main Area */}
                  <div>
                    {/* 1. Open Graph Image Banner (1200x630 format) for each card */}
                    <div className="relative aspect-[16/8.8] w-full bg-slate-900 overflow-hidden border-b border-slate-100">
                      <img
                        src={cardOgImage}
                      ...
                      <div className="absolute top-2.5 left-2.5 flex items-center gap-1.5">
                        <span className="px-2 py-0.5 rounded-md bg-black/60 backdrop-blur-md text-[10px] font-mono font-bold text-white border border-white/20 flex items-center gap-1">
                      ...
                      {isUploadSuccess && (
                        <div className="absolute inset-0 bg-emerald-950/80 backdrop-blur-xs flex items-center justify-center gap-1.5 text-white text-xs font-bold animate-in fade-in">
                          <Check className="w-4 h-4 text-emerald-400" />
                          <span>OG JPG Berjaya Dikemaskini!</span>
                        </div>
                      )}
                    </div>

                    {/* Dedicated Open Graph Upload Drop Area (Visible when Admin Mode active or card expanded) */}
                    {isAdminMode && (
                      <div className="p-3 bg-amber-50/70 border-b border-amber-100 text-xs">
                        <div className="flex items-center justify-between gap-2 mb-1.5">
                          <span className="font-bold text-amber-950 text-[11px] flex items-center gap-1">
                            ...
                            Ruang Upload Open Graph Image (JPG):
                          </span>
                          {customImage && (
                            <button
                        ...
                        <label className="flex items-center justify-center gap-2 p-2 rounded-xl bg-white border border-dashed border-amber-300 hover:border-[#0056D2] hover:bg-blue-50/50 transition-all cursor-pointer">
                          <ImageIcon className="w-3.5 h-3.5 text-[#0056D2]" />
                          <span className="text-[11px] font-semibold text-slate-700">
                            Pilih Fail JPG (1200×630)
                          </span>
                          <input
                            type="file"
                            accept="image/jpeg, image/jpg, image/png"
                    ...
                    >
                      ...
                      <div className="flex items-start justify-between gap-4 mb-3">
                        ...
                        <div className="flex items-center gap-2">
                          <span className="px-2.5 py-0.5 rounded-full text-[11px] font-bold border border-slate-200/80 bg-slate-50 text-slate-700">
                            {item.category}
                          </span>
                        </div>
                      </div>
                  ...
                  <div 
                    onClick={() => onSelectPlatform(item)}
                    className="px-5 pb-5 pt-1 flex items-center justify-between text-xs font-bold text-[#0056D2] group-hover:text-blue-700 cursor-pointer"
                  >
                    <span className="inline-flex items-center gap-1.5">
                      Terokai & Butiran
                      <ArrowRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" />
                    </span>
                    <span className="text-[10px] text-slate-400 font-normal">1200×630 JPG</span>
                  </div>
